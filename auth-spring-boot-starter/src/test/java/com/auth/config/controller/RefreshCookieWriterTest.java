...
class RefreshCookieWriterTest {
	...
	void write_ReturnsBase_WhenCookieDisabled() {
		...
		long refreshSeconds = 3600;
		RefreshCookieWriter writer = new RefreshCookieWriter(props, refreshSeconds);
		String body = "access-token";
		ResponseEntity<String> base = ResponseEntity.ok(body);

		ResponseEntity<String> result = writer.write(new Tokens("access-token", "refresh-token"), base);
	...
	void write_SetsRefreshCookieHeader_WhenCookieEnabled() {
		...
		long refreshSeconds = 1234;
		RefreshCookieWriter writer = new RefreshCookieWriter(props, refreshSeconds);
		String body = "access-token";
		ResponseEntity<String> base = ResponseEntity.status(HttpStatus.CREATED).body(body);
