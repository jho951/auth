...
이 문서는 **현재 구현 기준**으로 `boot-support`를 사용하는 서비스 애플리케이션에서 GitHub 로그인과 JWT/refresh cookie 발급을 연결하는 최소 예시입니다.
...
public class GitHubOAuth2PrincipalResolver implements OAuth2PrincipalResolver {
    ...
    public GitHubOAuth2PrincipalResolver(UserRepository userRepository) {
